...
which gives us the capacity of keeping all unique messages as of queue,
 as well as the capacity to quickly evaluate the duplication of messages as of set.
...
As one can see from the above example, there is an entry in the hashtable with the message m2,
 and the timestamp 2. Then there comes another message m2 with the timestamp 15.
Since the message was printed 13 seconds before (i.e. beyond the buffer window),
it is therefore eligible to print again the message.
...
Note: for clarity, we separate the two cases into two blocks.
One could combine the two blocks together to have a more concise solution.

The main difference between this approach with hashtable,
 and the previous approach with queue is that in previous approach we do proactive cleaning,
 i.e. at each invocation of function, we first remove those expired messages.

While in this approach, we keep all the messages even when they are expired.
This characteristics might become problematic,
...
class Logger:
    ...
    def should_print_message(self, timestamp: int, message: str) -> bool:
        if message not in self.message_dictionary:
